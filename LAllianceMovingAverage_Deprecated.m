classdef LAllianceMovingAverage < handle
    ...
    properties
        ...
        taskTimes   = [1000 1200; 1000 1200];
        taskTimesMv = [1000 1200; 1000 1200];
    ...
    properties (Constant)
        % Mmotivation is stored as [robotId taskId# ]
        s_motivation = SparseHashtable(10);

        % Average performance is stored as [robotId taskType#]
        s_tau = SparseHashtable(10);

        % Tau numbers performance is stored as [robotId type taskType#]
        % type = 2 = max, type = 1 = min 
    ...
    methods
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % 
        %   LAllianceMovingAverage(configId)
        %   
        ...
        %
        function this = LAllianceMovingAverage(configId,robotId)
...
%            end
        ...
        %           
        function Reset(this)
            %this.taskTimes(:,2) = this.taskTimes(:,2).*this.avgDecay;
            ...
            %HACK - we need the moving average, so here we will decay the
            %use an iterative stochastic update rule to track the averages

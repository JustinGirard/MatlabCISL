classdef LAlliance < handle
    ...
    methods
        ...
        function gating = GetGating(this,targetProperties)
            ...
            % carriedBy is a col vector [0 1 2 10  3 0 0]'
            % that represents a task's assigned agent
            notReturned = abs(targetProperties(:,1) -1); %ignore returned tasks
            carriedBy = abs(targetProperties(:,4)); 
            
            % makes row vector [ 0 0 0 1 0 ]' where
            % a 1 stands for a 'carried by me' task
            carriedByMe = carriedBy-this.rstate.id;
            carriedByMe = carriedByMe == 0;
            
            % make a vector [ 1 1 0 1 0 ]
            % where a one means the task is carried period.
            carriedByNobody = carriedBy == 0;
            
            disp('developing gating');
            myId = this.rstate.id
           %  if(sum(carriedByMe ) == 1)
            %    disp('details');
                one = abs(targetProperties(:,1));
                four = targetProperties(:,4);
                carriedBy;
                notReturned;
                carriedByMe;
                carriedByNobody;
                [notReturned carriedByNobody one four carriedBy carriedByMe ]
            %end
                    % Robot Properties 
        % [ currentTarget rotationStep mass robotType reach] 
        % Target Properties       
        % [ isReturned weight type(1/2) carriedBy size lastRobotToCarry]
            if(sum(carriedByMe ) == 1) %if we carry a box
                % return motivation toward only our caried box
                gating = notReturned.*carriedByMe;   
            else %if we don't carry a box             
                % returned the tasks carried by nobody and not returned
                gating = notReturned.*carriedByNobody;
            end
